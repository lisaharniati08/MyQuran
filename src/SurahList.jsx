    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm">114 Surat dalam Al-Qur'an</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {!loading && filteredSurahs.map((surah, index) => (
          <Link key={surah.nomor} to={`/surah/${surah.nomor}`}>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.02 }} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-primary/30 transition-all flex items-center gap-4 group">
              <div className="w-12 h-12 bg-gray-50 flex items-center justify-center rounded-xl font-semibold text-gray-600 group-hover:bg-primary group-hover:text-white transition-colors">{surah.nomor}</div>
              <div className="flex-1"><h3 className="font-semibold text-gray-800 group-hover:text-primary transition-colors">{surah.namaLatin}</h3><p className="text-xs text-gray-500">{surah.arti} • {surah.jumlahAyat} Ayat</p></div>
